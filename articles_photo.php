...
if( isset( $_FILES['photo'] ) or ( isset( $_POST['cutline'] ) and $_POST['cutline'] ) )
{
  ...
  if( $_POST['cutline'] )
  {
    ...
    $query = 'UPDATE articles SET
      cutline = "'.mysqli_real_escape_string( $connect, $_POST['cutline'] ).'"
      WHERE id = '.$_GET['id'].'
      LIMIT 1';
    mysqli_query( $connect, $query );
...
<form method="post" enctype="multipart/form-data">
  ...
  <label for="cutline">Cutline:</label>
  <input type="text" name="cutline" id="cutline" value="<?php echo htmlentities( $record['cutline'] ); ?>">
