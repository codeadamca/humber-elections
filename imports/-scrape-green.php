...
while( $next = strpos( $html, 'src=&quot;', $start ) and $counter < 500 )
{

  $firstQuote = $next + 10;
  $secondQuote = strpos( $html, '&quot;', $next + 11 );
  ...
  $firstH2 = strpos( $html, 'modal-title', $start ) + 13;
  $secondH2 = strpos( $html, ' in ', $firstH2 + 1 );
  ...
  if( mysqli_num_rows( $result ) == 0 )
  {
    ...
    $query = 'SELECT id,photo
      FROM candidates
      WHERE last LIKE "%'.$name[1].'%"
      AND partyId = 2259
      AND photo = ""
      LIMIT 1';
    $result = mysqli_query( $connect, $query );
    
    echo 'USING LAST AND PARTY<br>';
  }
